...
import { Search, Star, MapPin, Award, CheckCircle2, UserCheck, X, FileText, Calendar } from "lucide-react";
...
export default function TrainersDirectoryPage() {
  ...
  const { data: trainers = [], isLoading } = useTrainers({
    technology: techQuery || undefined,
  });
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-blue-500/10 via-cyan-500/10 to-purple-500/10 border border-blue-500/30">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative flex-1 w-full max-w-md">
            ...
            <input
              ...
              className="w-full bg-white/[0.04] border border-white/15 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-gray-400 focus:outline-none focus:border-cyan-400 transition"
            />
          </div>
              ...
              <GlassCard key={t.id} glowColor="cyan" className="flex flex-col justify-between p-6">
                <div>
                  <div className="flex items-start space-x-4 mb-4">
                    <img
                      src={t.photo || t.avatar_url || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=200"}
                      alt={t.name}
                ...
                <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                  <div>
                    ...
                    <span className="font-mono text-sm font-bold text-emerald-400">
                      {formatCurrency(t.hourlyRate || t.hourly_rate || 3500)}/day
                    </span>
                  </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className="w-full max-w-2xl bg-[#0D0E15] border border-cyan-500/40 rounded-2xl shadow-2xl p-6 relative glass-panel">
              ...
              <div className="flex items-start space-x-4 mb-6">
                <img
                  src={selectedTrainer.photo || selectedTrainer.avatar_url}
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3 text-xs text-gray-400 mt-1">
                    <span className="flex items-center text-amber-400 font-bold">
                      ...
                      {selectedTrainer.rating} ({selectedTrainer.totalTrainings || 48} Bootcamps)
                    </span>
                    <span>•</span>
              ...
              <div className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(selectedTrainer.certifications || ["AWS Certified ML Specialist", "NVIDIA DLI"]).map((c: string) => (
                      <span key={c} className="px-2.5 py-1 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-cyan-300">
                        {c}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <p className="text-gray-400">
                    {(selectedTrainer.pastColleges || ["IIT Bombay", "BITS Pilani", "IIIT Hyderabad"]).join(" • ")}
                  </p>
                </div>
              </div>
